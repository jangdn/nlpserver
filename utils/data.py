...
class SQuAD():
	...
	def preprocess_file(self,path):
		dump = []
		abnormals = [' ', '\n', '\u3000', '\u202f', '\u2009'] # what is thaht? Questions
		with open(path,'r',encoding ='utf-8') as f :
			data = json.load(f) # load data 
			data = data['data'] # train-v1.1.json all data in the data.json
			# data jsons file is { 'data': [data 1 ,data2]}
			for article in data :
				for paragraph in article['paragraphs']:
					...
					for qa in paragraph['qas']:
						...
						for ans in qa['answers']:
							...
							for i,t in enumerate(tokens): # why this part is neccsary?
								while l < len(context): ## What is this part
			...
			with open(f'{path}l','w',encoding = 'utf-8') as f :
				for line in dump:
					json.dump(line,f)
					print ('',file=f) # just write a something
