...
class batch:
    def __init__(self, c_char, c_word, q_char, q_word):
        self.c_char = c_char
        self.c_word = c_word
        self.q_char = q_char
        self.q_word = q_word
...
class SQuAD():
    def __init__(self, args):
        ...
        self.CHAR_NESTING = data.Field(batch_first=True, tokenize=list, lower=True,fix_length=40)
    ...
    def preprocess_input(self, args, context, query):
        c_char = self.CHAR.process([self.CHAR.preprocess(context)], device=args.gpu, train=False)
        c_word = self.WORD.process([self.WORD.preprocess(context)], device=args.gpu, train=False)
        q_char = self.CHAR.process([self.CHAR.preprocess(query)], device=args.gpu, train=False)
        q_word = self.WORD.process([self.WORD.preprocess(query)], device=args.gpu, train=False)

        input = batch(c_char, c_word, q_char, q_word)
